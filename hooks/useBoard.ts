  ...
  const moveJob = (jobId: string | number, targetId: string | number) => {
    ...
    // Case #2: Target is a job
      ...
      if (targetColumn === originalColumn) {
        const targetJobIndex = originalColumn.jobApplications.findIndex(
    ...
    // NOW we query newBoard instead of board, because board still has the old state
    const newColumn = newBoard.columns.find((col) =>
      col.jobApplications.some((job) => job._id === jobId),
    );
    const newColumnId = newColumn?._id;
    const newOrder = newColumn?.jobApplications.findIndex(
      (job) => job._id === jobId,
    );
